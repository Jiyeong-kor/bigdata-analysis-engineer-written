(() => {
  ...
  function injectUpdateControl() {
    ...
    const header = page?.querySelector('.compact-header-card');
    if (!page || !header) return;

    const section = document.createElement('section');
    section.className = 'compact-section';
    section.dataset.appUpdateSection = '';
    section.innerHTML = `
      <div class="resume-card-v2">
        <div class="resume-row">
          <div>
            <strong>앱 업데이트</strong>
            <p>새 버전이 있으면 적용한 뒤 자동으로 다시 엽니다. 삭제하거나 재설치할 필요가 없습니다.</p>
          </div>
          <button class="inline-link-button" data-action="app-update">업데이트</button>
        </div>
      </div>`;
